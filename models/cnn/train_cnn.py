...
from .cnn_model import CNN1DPredictor
...
def train_cnn(
    ...
    dl_tr, dl_va, dl_te, scaler, feature_cols = build_dataloaders(
        ...
        split=split,
        task=task,
        shuffle_train=True,  # allowed for window samples; just don’t mix time periods  [oai_citation:7‡Building a 1D CNN for Stock Prediction Using OHLCV Time-Series Features.pdf](sediment://file_000000002dd8720c983443cd1778dcb6)
    )

    in_channels = len(feature_cols)
    mk = model_kwargs or {}
    model = CNN1DPredictor(in_channels=in_channels, task=task, **mk).to(device)
