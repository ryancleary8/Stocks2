"""Predict next-day open price/direction for XGBoost models only.

This script mirrors ``predict.py`` but restricts inference to XGBoost
models. It expects one CSV per symbol inside the ``current`` directory
with filename ``{SYMBOL}.{YYYY-MM-DD}.csv`` containing columns
``timestamp_utc,symbol,price`` produced by ``scraper.py``. Historical
OHLCV data must already exist (e.g., in ``processed/`` or ``data/clean/``)
so features can be computed up to the latest open price. Models are
...
def _gather_model_paths(symbol: str) -> dict[str, Path]:
    """Return mapping of model-key -> path for a symbol limited to XGBoost."""
    ...
    def scan(root: Path) -> Iterable[Path]:
        ...
        allowed_suffixes = {".joblib", ".pkl", ".pickle", ".json", ".ubj"}
    ...
    xgb_paths: dict[str, Path] = {}
    for key, path in paths.items():
        meta = _load_model_meta(path.parent)
        model_type = str(meta.get("type", "")).lower()
        if "xgb" in key.lower() or "xgb" in model_type:
            xgb_paths[key] = path
...
def predict_for_symbol(quote: CurrentQuote) -> dict[str, float]:
    ...
    if not model_paths:
        print(f"⚠️  No XGBoost models found for {quote.symbol}")
        return preds
    ...
    for key, model_path in sorted(model_paths.items()):
        ...
        try:
            ...
            is_classifier = ("cls" in key.lower() or "classifier" in model_type or task == "cls")
            ...
            elif is_classifier or model_type == "xgb_classifier" or "xgb_cls" in key.lower():
...
def main() -> None:
    ...
    ordered_keys = [
        ...
        "open_price",
        "open_gt_prev_close",
        *sorted(k for k in all_keys if k not in {"symbol", "open_price", "open_gt_prev_close"}),
    ]
